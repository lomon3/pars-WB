...
def get_data(url):
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Bad response for URL: {url}. Status code: {response.status_code}")
    except requests.exceptions.RequestException as e:
        print(f"RequestException for URL: {url}. Exception: {str(e)}")
...
for i in range(10):
    df[f'SKU{i+1}'] = df['data_skus'].apply(lambda x: x[i] if isinstance(x, list) and len(x) > i else None)
